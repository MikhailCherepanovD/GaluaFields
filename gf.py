...
def gf_poly_scale(p, x):                                                   # умножение вектора на константу
    res = [mult(coeff, x) for coeff in p]
    return res

def gf_poly_add(p, q):                                                     # сложение полиномов через ксор
    ...
    for i in range(len(q)):
        res[i + len(res) - len(q)] ^= q[i]

    return res


def gf_poly_mult(p, q):
    res = [0] * (len(p) + len(q) - 1)

    for j in range(len(q)):
        for i in range(len(p)):
            res[i + j] ^= mult(p[i], q[j])
    ...
    for i in range(len(dividend) - (len(divisor) - 1)):
        ...
        if coef != 0:
            for j in range(1, len(divisor)):
                if divisor[j] != 0:
                    remainder[i + j] ^= mult(divisor[j], coef)
...
# Вызов функции
quotient, remainder = gf_poly_div(dividend, divisor)
